...
## A generic example of HMM: the bags and balls' colors series
...
Let's consider the following example (which we will refer to as the **bags and balls's colors example*}** throughout this tutorial).
...
Finally we know the series $X=(X_1,X_2,X_3,...,X_n)$ which corresponds, in the bags and balls' colors example, to the **balls' colors** (i.e. the **observations** series). The two possible **outcomes** are "red"  (R) or "white" (W). On the other hand, we do not know the series $S=(S_1,S_2,S_3,...S_n)$ which corresponds to the **names of the bags** out of which the balls have been drawn (i.e. the **hidden states** series)
...
Answering the first question consists in fitting the model, i.e. estimating its parameters (emission and transition probabilities). This will be the focus of section \ref{baumwelch}.

Answering the second one consists in calculating the most likely series of hidden states (based on both observation series and fitted model). This will be the focus of section \ref{viterbi}.
