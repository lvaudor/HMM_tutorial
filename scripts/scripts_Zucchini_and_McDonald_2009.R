#pois.HMM.pn2pw transforms the natural parameters (the vector of
#Poisson means λ and the t.p.m. Γ) of an m-state Poisson–HMM into
#a vector of (unconstrained) working parameters;
#• pois.HMM.pw2pn performs the inverse transformation;
#• pois.HMM.mllk computes minus the log-likelihood of a Poisson–HMM
#for a given set of working parameters; and
#• pois.HMM.mle estimates the parameters of the model using numerical
...
pois.HMM.mllk=function(parvect,x,m ,...){
  ...
  lscale<-0
  foo<-pn$delta
  for(i in 1:n){# forward probability
...
#######################################################################################
#### EM estimation of a Poisson–HMM
